<template>
  <div
    v-if="currentIndex > 0"
    class="fixed left-1/2 top-1/2 flex max-h-[75vh] w-full max-w-lg -translate-x-1/2 -translate-y-1/2 flex-col gap-6"
  >
    <AnimatePresence multiple as="div" class="grid grow grid-cols-3 gap-x-4">
      <Podium
        v-if="sortedPlayersByScore.length >= 2"
        v-show="1 >= sortedPlayersByScore.length - currentIndex"
        class="col-span-1 col-start-1"
        :player="sortedPlayersByScore[1]!"
        :score="sortedPlayersByScore[1]?.score || 0"
        medal="silver"
      />
      <Podium
        v-if="sortedPlayersByScore.length >= 1"
        v-show="0 >= sortedPlayersByScore.length - currentIndex"
        class="col-span-1 col-start-2"
        :player="sortedPlayersByScore[0]!"
        :score="sortedPlayersByScore[0]?.score || 0"
        medal="gold"
      />
      <Podium
        v-if="sortedPlayersByScore.length >= 3"
        v-show="2 >= sortedPlayersByScore.length - currentIndex"
        class="col-span-1 col-start-3"
        :player="sortedPlayersByScore[2]!"
        :score="sortedPlayersByScore[2]?.score || 0"
        medal="bronze"
      />
    </AnimatePresence>
    <AnimatePresence
      v-if="sortedPlayersByScore.length >= 4"
      multiple
      as="div"
      class="flex flex-col gap-y-4 overflow-auto"
    >
      <template v-for="(player, index) in sortedPlayersByScore" :key="player.id">
        ...
        >
          ...
          <span class="ms-auto"
            >{{ player?.score }}
            {{ player?.score != null && player?.score > 1 ? "pts" : "pt" }}</span
          >
        </Motion>
      </template>
    </AnimatePresence>
  ...
  <h1
    v-else
    class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-16 text-xl font-semibold sm:text-3xl md:text-5xl"
  >
    Voici les résutats ...
  </h1>
  <NextButton
    ...
    :title="
      sortedPlayersByScore.length - currentIndex > 0
        ? 'Prochain joueur'
        : 'Retour à la page d\'accueil'
    "
    :description="`Voir la ${sortedPlayersByScore.length - currentIndex}ᵉ place`"
    @click="
      sortedPlayersByScore.length - currentIndex > 0 ? $emit('next-player') : $emit('end')
    "
  />
</template>
...
props.game.playersData.forEach((playerData) => {
  if (props.players.find((player) => player.id === playerData.id)) {
    const newPlayer: User = props.players.find(
