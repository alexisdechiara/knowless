  ...
  <div class="absolute inset-0 flex size-full flex-col gap-4 p-4">
    ...
    <div class="relative h-9 w-full">
      ...
      <template v-if="mode === 'solo' || phase === 'question'">
        <span
          class="absolute start-1/2 top-0 -translate-x-1/2 text-2xl font-semibold leading-none! md:text-3xl"
          >Question {{ nbQuestion }}</span
        >
      ...
      <template v-if="mode === 'multi' && phase === 'correction'">
        <span v-if="content?.points" class="absolute right-0 top-0 text-xl font-semibold"
          >{{ content.points }} {{ content.points > 1 ? "points" : "point" }}</span
        >
      </template>
      <slot name="header" :status="status" />
    </div>
    <div
      class="relative -mt-32 flex size-full flex-col items-center justify-center sm:-mt-20"
    >
      <div class="flex w-full flex-col gap-y-4 px-6 sm:max-w-md sm:px-0 md:max-w-lg">
        <span
          v-if="props.content?.theme"
          class="pb-4 text-center text-3xl font-bold sm:pb-6 sm:text-4xl md:pb-10 md:text-5xl"
        >
          {{ content?.theme }}
        </span>
        <NuxtImg
        ...
        <ToggleGroup
          ...
          :type="checkMultipleCorrectAnswers ? 'multiple' : 'single'"
          variant="outline"
          :class="{ 'pointer-events-none': showResult }"
          class="grid auto-rows-fr grid-cols-2 gap-4 w-full !shadow-none"
        >
          <ToggleGroupItem
          ...
          <Tooltip>
            <TooltipTrigger as-child>
              <Button
                ...
                size="icon"
                class="absolute bottom-28 left-0 flex size-fit rounded-full p-3 sm:bottom-40 sm:left-8"
            ...
            <Tooltip>
              <TooltipTrigger as-child>
                <Button
                  :variant="outlineOrSecondary()"
                  size="icon"
                  class="absolute bottom-14 left-0 flex size-fit rounded-full p-3 sm:bottom-24 sm:left-8"
                >
        ...
        <Tooltip>
          <TooltipTrigger as-child>
            <Button
              :variant="outlineOrSecondary()"
              size="icon"
              class="absolute bottom-0 left-0 flex size-fit rounded-full p-3 sm:bottom-8 sm:left-8"
              @click="
                navigateTo(content.wikipedia, {
                  external: true,
                  open: { target: '_blank' },
                })
              "
      ...
      <slot name="actions" v-if="showResult" />

      <template
        v-if="
          ((mode === 'solo' && status === 'correct') ||
            (mode === 'multi' && phase === 'correction')) &&
          showResult &&
          showNext
        "
      >
        ...
        <Button
          v-else
          variant="secondary"
          class="absolute bottom-0 right-0 flex h-fit py-4 pl-5 sm:bottom-8 sm:right-8"
          @click="emit('next')"
        >
          <div class="flex flex-col">
            <span class="text-xl font-medium">Suivant</span>
            <span v-if="nbQuestion" class="text-sm text-secondary-foreground/50"
              >Question {{ nbQuestion + 1 }}</span
            >
          </div>
      ...
      <div
        v-if="status === 'incorrect' && showResult"
        class="absolute bottom-0 right-0 mt-16 flex justify-between gap-x-2 sm:bottom-8 sm:right-8"
      >
        <Button
          v-if="showBack"
          size="icon"
          :variant="outlineOrSecondary()"
          class="px-7 py-6 text-xl"
          @click="emit('back')"
        >
...
const nbQuestion = computed(() =>
  props?.questionNumber || props?.questionNumber === 0
...
onUnmounted(() => {
  if (
    (inputAnswer.value === "" || inputAnswer.value == null) &&
    (selectedAnswer.value === "" || selectedAnswer.value == null)
  ) {
    emit("answer", "");
  } else {
    if (props.content?.type === "open" && emittedAnswer.value !== inputAnswer.value) {
      ...
      (props.content?.type === "four" || props.content?.type === "two") &&
      emittedAnswer.value !== selectedAnswer.value
    ) {
...
if (props.answer) {
  if (props.content?.type === "open") {
    ...
    selectedAnswer.value = String(
      props.content?.answers.findIndex((answer) => answer.value === props.answer)
    );
}
...
watchDebounced(
  selectedAnswer,
  () => {
    emittedAnswer.value = String(
      props.content?.answers[Number(selectedAnswer.value)]?.value
    );
...
const correctAnswers = computed(() => {
  ...
  return props.content.answers
    .filter(
      (answer): answer is Quizz["answers"][number] & { isCorrect: true } =>
        answer.isCorrect
    ) // Garde seulement les réponses correctes (avec type guard)
    .flatMap((answer) => {
...
const normalizedCorrectAnswers = computed(() =>
  correctAnswers.value.map((str) => normalize(str))
);
...
function evaluateOpenAnswerCorrectness(): boolean {
  if (results.value.length > 0) {
    ...
    if (bestMatchScore != null && bestMatchScore <= 0.3) {
      ...
      bestMatchScore != null &&
      bestMatchScore <= fuseOptions.value.fuseOptions.threshold
    ) {
...
const checkMultipleCorrectAnswers = computed(
  () =>
    props.content?.answers &&
    props.content.answers.filter((answer) => answer.isCorrect).length > 1
);
...
const remainingMilliseconds = computed(() =>
  Math.floor((remainingTime.value % 1000) / 10)
);

watch(
  () => (props as any).startAt,
  (newStartAt) => {
