				...
				<div class="flex size-full flex-col gap-y-8 md:grid md:grid-cols-5 md:gap-x-32">
					<div class="flex w-fit flex-col md:col-span-2">
						<div class="flex w-full max-w-lg items-center gap-4 overflow-auto md:mb-auto">
							<Avatar size="lg">
								<AvatarImage :src="getPlayer.avatar ? getPlayer.avatar : ''" alt="avatar" />
							...
							<div class="flex flex-col text-2xl leading-normal">
								...
								<span class="italic text-muted-foreground">{{ lobby?.host === getPlayer.id ? 'Hôte du salon' : 'Joueur' }}</span>
							</div>
						</div>
						<div id="settings" />
					</div>
...
onMounted(async () => {
	...
	if (error) {
		...
		if (getPlayer.id !== lobby.value?.host && !lobby.value?.playerIds.includes(getPlayer.id)) {
			navigateTo(`/multi/join`)
		}
		else if (lobby.value?.invitedPlayersId && lobby.value?.invitedPlayersId.includes(getPlayer.id) && lobby.value?.maxPlayers > lobby.value?.players.length) {
			navigateTo(`/multi/join`)
		}
