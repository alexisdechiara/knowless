...
<template>
	<component :is="breakpoints.greater('sm').value ? Card : 'div'" class="fixed left-1/2 top-1/2 flex size-full -translate-x-1/2 -translate-y-1/2 flex-col gap-4 p-6 sm:size-2/3 sm:shadow-md">
		<CardHeader v-if="$slots.header" class="flex p-0">
			<slot name="header" />
		</CardHeader>
		<CardContent class="flex size-full flex-col justify-center p-0">
		...
		<CardFooter v-if="back || $slots.footer" class="flex p-0">
			<Button v-if="back" variant="secondary" size="icon" @click="() => $router.back()">
